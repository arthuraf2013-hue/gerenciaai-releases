  ...
  return (
    <div className="modal-overlay">
      <div className="modal-card modal-card-fullscreen">
        <h2 style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><Icon name="clipboard" size={18} /> Novo orçamento</h2>
        ...
        <div className="modal-actions">
          <button className="btn-secondary" onClick={onCancelar}><Icon name="close" size={15} /> Fechar</button>
          {quoteId && itens.length > 0 && <button className="btn-primary" onClick={() => onCriado()}><Icon name="checkCircle" size={15} /> Concluir</button>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="screen">
      <div className="screen-header">
        <h1 style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><Icon name="clipboard" size={20} /> Orçamentos</h1>
        <button className="btn-primary" onClick={() => setShowNovo(true)}><Icon name="add" size={15} /> Novo orçamento</button>
      </div>
      ...
      {detalhe && (
        <div className="modal-overlay">
          <div className="modal-card">
            <h2 style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><Icon name="clipboard" size={18} /> Orçamento — {detalhe.clienteNome || 'sem cliente'}</h2>
            ...
            <div className="modal-actions">
              <button className="btn-secondary" onClick={() => setDetalhe(null)}><Icon name="close" size={15} /> Fechar</button>
              {detalhe.status === 'aberto' && (
                <>
                  <button className="btn-link-danger" onClick={() => handleCancelar(detalhe.id)}><Icon name="close" size={15} /> Cancelar orçamento</button>
                  <button className="btn-primary" onClick={() => handleConverter(detalhe.id)}><Icon name="checkCircle" size={15} /> Converter em venda</button>
                </>
              )}
            </div>
          </div>
