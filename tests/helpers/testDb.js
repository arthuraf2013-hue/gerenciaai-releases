...
function createProduct(db, { nome = 'Produto Teste', preco = 10, estoqueMinimo = 5 } = {}) {
  const id = randomUUID();
  db.prepare(
    `INSERT INTO products (id, nome, preco, estoque_minimo, unidade) VALUES (?, ?, ?, ?, 'un')`
  ).run(id, nome, preco, estoqueMinimo);
  return id;
}
